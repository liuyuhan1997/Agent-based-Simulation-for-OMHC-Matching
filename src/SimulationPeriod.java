...
public class SimulationPeriod {
    ...
    SimulationPeriod(int i, int currentMemberCount, int currentListenerCount) {
        /**
         * creating new members and listeners to make pool consistent with user
         * distribution
         **/
        this.index = i;
    ...
    public void FirstComeFirstServe() {
        ...
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortMemberByWaiting(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }
    ...
    public void ShortWaitTime() {
        ...
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortMemberByWaitingOpposite(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }

    public void SimilarityAgeGender() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SortBySimilarity(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortBySimilarity(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }

    public void SoftGender() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SoftSortListenerByGender(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SoftSortMemberByGender(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }
    ...
    public void SoftAge() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SoftSortListenerByAge(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SoftSortMemberByAge(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }
    ...
    public void SoftExp() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SoftSortListenerByExp(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SoftSortMemberByExp(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }

    public void TopicMatching() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SortListenerByTopic(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortMemberByTopic(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }

    public void SimilarityAll() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SortBySimilarityWithTopic(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortBySimilarityWithTopic(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }

    public void SimilarityWithoutExp() {
        for (Member m : activeMemberPool) {
            Collections.sort(activeListenerPool, new SortSimilarityWithoutExp(m));
            m.preferenceList = (ArrayList<Listener>) activeListenerPool.clone();
        }
        for (Listener l : activeListenerPool) {
            Collections.sort(activeMemberPool, new SortSimilarityWithoutExp(l));
            l.preferenceList = (ArrayList<Member>) activeMemberPool.clone();
        }
    }
    ...
    public void writeMatchedPairs(Pair p) {
        ...
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter("similarity/Matched.csv", true));
            out.write(this.index + "," + p.member.id + "," + p.member.birthyear + "," + p.member.gender + ","
                    + p.member.tenure + "," + p.member.experience + "," + p.listener.id + "," + p.listener.birthyear
                    + "," + p.listener.gender + "," + p.listener.tenure + "," + p.listener.experience + "," + p.length
                    + "," + p.rating + "," + p.blocking + "," + (p.member.waitTime) + "," + p.listener.decisionTime
                    + "," + listener_expertise_match + "," + p.cosineSimilarity + "\n");
            out.close();
        } catch (IOException e) {
        }
    }
    ...
    public void writeQuitMembers(Member member) {
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter("similarity/UnmatchedMembers.csv", true));
            out.write(this.index + "," + member.id + "," + member.birthyear + "," + member.gender + "," + member.tenure
                    + "," + member.experience + "," + member.patience + "," + member.waitTime + "," + member.topic
                    + "\n");
            out.close();
        } catch (IOException e) {
        }
    ...
    public void writeCurrentMin() {
        try {
            BufferedWriter out = new BufferedWriter(new FileWriter("similarity/Mins.csv", true));
            out.write(this.index + "," + this.memberCount + "," + this.listenerCount + "," + Machi.memberPool.size()
                    + "," + Machi.listenerPool.size() + "," + activeMemberPool.size() + "," + activeListenerPool.size()
                    + "\n");
            out.close();
        } catch (IOException e) {
        }
